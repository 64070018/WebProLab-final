...
const { isLoggedIn } = require('../middlewares'); //import func[middleware] from floder middlewares
...
router.post('/user/login', async (req, res, next) => {
    ...
    try {
        ...
        // bcrypt มีการ encript เอาไว้ อย่าเก็บเป็น paintext
        ...
        let token = tokens[0]?.token // ส่ง token เดิมคืนให้ไปเลย
        if (!token) {
            // Generate and save token into database
            token = generateToken() // ยังไม่เคยมี token generate ใหม่
            await conn.query(
