...
router.post("/blogs", upload.single('myImage'), async function (req, res, next) {
  ...
  // have 2 query if not transaction then confuse data
  const conn = await pool.getConnection()
  await conn.beginTransaction(); // บอกว่าจะเริ่มคิวรี่เยอะๆ แล้วนะ ถ้าพังให้ rollback กลับมาตอนนี้(สถานะก่อนคิวรี่)
  // ดาต้าเบสจะจำสถานะไว้ให้
  ...
  try {
    ...
    await conn.commit() // บอกว่า transaction นี้เสร็จเรียบร้อยสามารถ save data to db ได้เลย แต่ยังไม่จบ จบตอน release
    res.redirect('/')

    // res.send("success!");
  } catch (err) {
    await conn.rollback(); // ถ้า error มาให้กลับไปตอนเริ่ม transaction
    next(err);
  } finally {
    console.log('finally')
    conn.release(); // จบการทำงาน ทิ้งไปได้เลย
  }
  // if commit is complate can't rollback
...
router.get("/blogs/:id", async function (req, res, next) {
  const promise1 = await pool.query("SELECT * FROM blogs WHERE id=?", [
    req.params.id,
  ]);
  const promise2 = await pool.query("SELECT * FROM comments WHERE blog_id=?", [
    req.params.id,
  ]);
  const promise3 = await pool.query("SELECT * FROM images WHERE blog_id=? and comment_id is null", [
    req.params.id,
  ]);

  const promise4 = await pool.query("select * from comments c, images i where c.id = i.comment_id")

  const promise = await pool.query("select * FROM comments left outer join images on (comments.id = images.comment_id) WHERE comments.blog_id = ?",
    [req.params.id])

  Promise.all([promise1, promise2, promise3, promise4, promise])
    .then((results) => {
      const blogs = results[0];
      const comments = results[1];
      const images = results[2];
      const commentImage = results[3];
      const newComments = results[4];
      res.render("blogs/detail", {
        blog: blogs[0][0],
        comments: comments[0],
        images: images[0],
        commentImage: commentImage[0][0],
