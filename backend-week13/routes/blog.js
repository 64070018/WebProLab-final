...
//   then: Joi.date().less(Joi.ref('end_date')).required(),
//   otherwise: Joi.date().forbidden()
// }),

const createBlogSchema = Joi.object({
  title: Joi.string().required().min(10).max(25).pattern(/^[a-zA-Z]+$/),
  jim: Joi.string().required().min(5),
  statusBlog: Joi.string().required().valid('status_private', 'status_public'),
...
router.post("/blogs", upload.array("myImage", 5), async function (req, res, next) {
  if (req.method == "POST") {
    ...
    console.log(req.body.statusBlog)
    ...
    const jim = req.body.jim;
    let status = req.body.statusBlog;
